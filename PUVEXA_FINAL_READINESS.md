...
LIVE_AI_TEST=BLOCKED_OPENAI_NO_CREDITS (AI_PROVIDER=openai + API key now configured; real provider call returned HTTP 429 "You have no credits remaining"; live methods implemented — image/log/code diagnosis via real vision/chat calls, ranking, verification, contribution scoring — and provider-independent redaction/compaction gates PASS; pending account credit)
...
READY_ENDPOINT=PASS (live GET /ready returns 200 status=ready, database=ok, auth/storage=configured, ai_provider=not_configured, chain/contracts=disabled — exact reflection of configured env)
